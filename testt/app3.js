...
const CART_DATA = [
  ...
  function funkcija(CART_DATA) {
  const itemi = CART_DATA.map(element=> element.amount).reduce
    ((a,b)=>a+b);
    const postarina = 320;
    const barca = CART_DATA.map(element => {
       return{
           name: element.name,
           total: element.price * element.amount
       }
    })
    const united = barca.map(element=> element.total)
    
    const klopp = united.reduce((a,b)=> 
       a+b,postarina
    )
